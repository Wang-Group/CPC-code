...
expratio=expdata(:,3)';
select1=find(exp(water)+1.75*exp(formic)>0.6125); %to match the region for solubilibty measurement
% select1=1:93;
...
%% several parameters
% global HfSBUdis crystalsize(k,l) centropos Hf12vsHf6 pHf12 SBUpos SBUpos2 potentialSBUconnect potentialSBUconnect2 direction 
delta=0;%delta
n1=10;
Hf12overHf6rate=10;
addprobability=0.01;
...
Maxcrystalsize=40;
TimeSteps=1000000;
nucleationsize = 1; %set the initial step to nucleation



% Maxcrystalsize = 1000; %set the number limit of the SBU in crystal
addsequence=-0.5:0.1:0.5;
...
for k=1:Numpoints
    ...
    getrid=floor(0.12*length(Nucleationtime(k,:)));
    nuclearate(k)=1./mean(timecollect(getrid+1:end-getrid));
end
figure_FontSize = 14;
figure
plot(H2BPDC/2.303,log(nuclearate)/2.303,'b*');
ylabel('log (nucleation rate)','fontsize',figure_FontSize,'fontname','Arial')
xlabel('log (x[H_{2}BPDC])','fontsize',figure_FontSize,'fontname','Arial')
...
xlabel('log (x[H_{2}BPDC])','fontsize',figure_FontSize,'fontname','Arial')
set(gca,'fontsize',figure_FontSize,'fontname','Arial')
[p s] = polyfit((H2BPDC)'/2.303,log(nuclearate)/2.303,1);
[y dy]=polyval(p,(H2BPDC)/2.303,s);
hold on
plot(H2BPDC/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
legend('boxoff')
set(gca,'Xtick',-4.4:0.2:-3.9,'Ytick',-2.35:0.01:-2.25);

save(strcat('comparethenucleationstep20190227_H2BPDCdenpendent_homogeneous_delta',num2str(delta),'.mat'),'-v7.3');
