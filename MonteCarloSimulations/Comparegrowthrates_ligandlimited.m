...
for l=1:repeats
for k=select1(7:end) % k represents the k th point
...
pause(0.01);
save(strcat('comparegrowthrates20191109_delta=',num2str(delta),'.mat'));
...
growthrate(k)=mean(th(isnan(th)==0));
aspect_ratio(k)=mean(as(isnan(as)==0));  
end

plot(growthrate(select1),expthickness(select1),'*');
