...
water = water(select1(1))+addsequence;
...
for i=1:length(water)
    ...
    stdgrowthrate=std(log(getgrowthrate(:,9:end-8)),0,2);
    getgrowthrate=mean(getgrowthrate(:,9:end-8),2);
end


figure
plot(water/2.303,log(getgrowthrate)/2.303,'b*');
figure_FontSize = 14;
ylabel('log (crystal growth rate)','fontsize',figure_FontSize,'fontname','Arial')
xlabel('log (x[H_{2}O])','fontsize',figure_FontSize,'fontname','Arial')
set(gca,'XTick',-2.0:0.2:-1.6);
set(gca,'YTick',-0.3:0.02:-0.15);
set(gca,'fontsize',figure_FontSize,'fontname','Arial')

[p s] = polyfit((water)'/2.303,log(getgrowthrate)/2.303,1);
[y dy]=polyval(p,(water)/2.303,s);
hold on
plot(water/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
legend('boxoff')
data2save=[water' getgrowthrate];

save('comparegrowthrates_H2O.txt','-ascii','data2save');
save('H2Odependent.mat')
