...
figure_FontSize = 14;
figure
plot(formate_formic/2.303,log(nuclearate)/2.303,'b*');
ylabel('log (nucleation rate)','fontsize',figure_FontSize,'fontname','Arial')
xlabel('log ([HCO_{2}^{-}]/[HCO_{2}H])','fontsize',figure_FontSize,'fontname','Arial')
...
xlabel('log ([HCO_{2}^{-}]/[HCO_{2}H])','fontsize',figure_FontSize,'fontname','Arial')
set(gca,'fontsize',figure_FontSize,'fontname','Arial')
[p s] = polyfit((formate_formic)'/2.303,log(nuclearate)/2.303,1);
[y dy]=polyval(p,(formate_formic)/2.303,s);
hold on
plot(formate_formic/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
set(gca,'Xtick',-1.7:0.2:-1.2,'Ytick',-2.26:0.002:-2.25);
legend('boxoff')

save(strcat('comparethenucleationstep20190227_formate_denpendent_delta',num2str(delta),'.mat'),'-v7.3');
