...
figure
plot(formate_formic/2.303,log(getgrowthrate)/2.303,'b*');
figure_FontSize = 14;
...
set(gca,'XTick',-1.9:0.2:1.4);
set(gca,'YTick',-0.4:0.05:-0.2);
set(gca,'fontsize',figure_FontSize,'fontname','Arial')

[p s] = polyfit((formate_formic)'/2.303,log(getgrowthrate)/2.303,1);
[y delta]=polyval(p,(formate_formic)/2.303,s);
hold on
plot(formate_formic/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
legend('boxoff')
data2save=[formate_formic' getgrowthrate];

save('comparegrowthrates_formate.txt','-ascii','data2save');
save('formate_dependent.mat')
