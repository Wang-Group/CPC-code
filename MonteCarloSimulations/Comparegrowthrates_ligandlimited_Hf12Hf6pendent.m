...
Hf12_Hf6 = Hf12_Hf6(select1(1))+addsequence;
...
for i=1:length(H2BPDC)
    getgrowthrate=sort(crystalgrowth,2);
    stdgrowthrate=std(log(getgrowthrate(:,3:end-2)),0,2);
    getgrowthrate=mean(getgrowthrate(:,3:end-2),2);
end

figure
plot(Hf12_Hf6/2.303,log(getgrowthrate)/2.303,'b*');
figure_FontSize = 14;
ylabel('log (crystal growth rate)','fontsize',figure_FontSize,'fontname','Arial')
xlabel('log ([Hf_{12}]/[Hf_{6}])','fontsize',figure_FontSize,'fontname','Arial')
set(gca,'XTick',-0.2:0.2:0.3);
set(gca,'YTick',-0.26:0.02:-0.2);
set(gca,'fontsize',figure_FontSize,'fontname','Arial')

[p s] = polyfit((Hf12_Hf6)'/2.303,log(getgrowthrate)/2.303,1);
[y delta]=polyval(p,(Hf12_Hf6)/2.303,s);
hold on
plot(Hf12_Hf6/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
legend('boxoff')
data2save=[Hf12_Hf6' getgrowthrate];

save('comparegrowthrates_Hf12Hf6.txt','-ascii','data2save');
save('Hf12_Hf6dependent.mat')
