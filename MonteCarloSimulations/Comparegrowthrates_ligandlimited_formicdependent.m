...
for i=1:length(formic)
    getgrowthrate=sort(crystalgrowth,2);
    stdgrowthrate=std(log(getgrowthrate(:,2:end-1)),0,2);
    getgrowthrate=mean(getgrowthrate(:,2:end-1),2);
end

figure
plot(formic/2.303,log(getgrowthrate)/2.303,'b*');
figure_FontSize = 14;
...
[p s] = polyfit((formic(1:end-3)')/2.303,log(getgrowthrate(1:end-3))/2.303,1);
[y dy]=polyval(p,(formic)/2.303,s);
hold on
plot(formic/2.303,y,'r','linewidth',2.5);
legend('Average over 50 runs',strcat('Slope = ',num2str(p(1),2)),'fontsize',16)
legend('boxoff')
data2save=[formic' getgrowthrate];

save('comparegrowthrates_formic.txt','-ascii','data2save');
save('formicdependent.mat')
